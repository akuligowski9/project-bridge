...
### DEC-018: Curated data over AI for doc links and features

**Date:** 2026-02-14
**Status:** Accepted
**Context:** The `export-project` feature needs official documentation links and per-difficulty feature suggestions for ~50 skills. These could be generated by AI on each invocation or curated once in data files.
**Decision:** Use curated YAML data files (`resources.yaml` for doc links, `skill_features.yaml` for per-skill per-difficulty features). Doc links are never AI-generated. Features use curated data with generic fallback. AI is used only for the description and career context (with heuristic fallback).
**Reasoning:** Doc links must point to real, current official URLs — hallucinated links would undermine user trust. Per-difficulty features benefit from human curation to ensure they're genuinely demonstrable and appropriately scoped. Curating once and caching is also faster and cheaper than generating on every invocation. The AI path adds value for personalized description writing where creativity matters, but structured data is better served by structured files.
**Alternatives Considered:** Fully AI-generated specs (risk of hallucinated URLs and vague features), hybrid with AI-generated features validated against a whitelist (complexity without proportional value).
**Consequences:** New skills require manual additions to both YAML files. The generic fallback ensures unknown skills still produce usable output. AI providers enhance the description quality but the spec is useful even with `--no-ai`.
